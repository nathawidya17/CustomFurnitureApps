...
function loadMainModel() {
    ...
    showModelStatus(currentModelEntry ? `Loading ${currentModelEntry.name}...` : 'Loading model...');
    if (!currentModelPath) {
        console.warn('No model selected; skipping load');
        showModelStatus('No model selected — pilih dari katalog', true);
        return;
    }
    ...
    loader.load(
        ...
        (gltf) => {
            ...
            showModelStatus(`Model loaded — ${currentModelEntry ? currentModelEntry.name : ''}`);
            adjustCameraPosition();

            // Reveal controls (now that a model is loaded)
            const controlsEl = document.getElementById('controls');
            if (controlsEl) controlsEl.style.display = 'flex';
            // add a "Back to catalog" button in the controls area if not present
            if (!document.getElementById('backToCatalogBtn')) {
                const controlsArea = document.getElementById('controls');
                const btn = document.createElement('button');
                btn.id = 'backToCatalogBtn';
                btn.textContent = '← Katalog';
                Object.assign(btn.style, {marginTop:'8px'});
                btn.addEventListener('click', () => {
                    returnToCatalog();
                });
                if (controlsArea) {
                    controlsArea.prepend(btn);
                } else {
                    document.body.appendChild(btn);
                }
            }

            // Hide the catalog if it was visible
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ensure rack control visibility initial state
    ...
    function showCatalog() {
        ...
        // hide controls while catalog shown
    ...
    function selectModel(entry) {
        ...
        currentModelPath = entry.file;
        // rack mode detection
        isRackMode = (entry && entry.type === 'rack');
        // reset user change flag until user moves sliders
        userChangedConfig = false;
        // enable auto-fit for the upcoming model load (one-time)
        cameraAutoFitEnabled = true;
    // reset layout counts
    rackCols = 1; rackRows = 1;
    if (document.getElementById('rackCols')) {
        document.getElementById('rackCols').value = rackCols;
        document.getElementById('rackRows').value = rackRows;
        document.getElementById('rackColsValue').textContent = rackCols;
        document.getElementById('rackRowsValue').textContent = rackRows;
    }
    // show/hide rack-specific controls
    try { updateRackControlsVisibility(); } catch(e){}
    hideCatalog();
    ...
    function returnToCatalog() {
        // Remove loaded model and show catalog again
        if (loadedModel) {
            mainCabinet.remove(loadedModel);
            loadedModel = null;
            modelOriginalBox = null;
        ...
        // remove back button
        const btn = document.getElementById('backToCatalogBtn'); if (btn) btn.remove();
        // hide any status
        showModelStatus('');
        showCatalog();
    }
